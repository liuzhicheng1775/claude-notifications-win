...
> 推荐直接用 `notify.exe init` 向导配置，下面是手动配置说明。详细步骤见 [docs/feishu-notification.md](./docs/feishu-notification.md)。
